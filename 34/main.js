...
const filterList = document.querySelector('.filter-list'); //фильтрованный список
const contentHolder = document.querySelector('.content-holder'); //общий список
const input = document.querySelector(".search-input"); //обозначаем input в js

input.addEventListener('keydown', (event) => {
    if (event.keyCode === 13) {
        let filteredMust = mustangs.filter(el => el.model === input.value);
        filteredMust.map((item) => {
            const block = `
            <div class="item">
...
//             });
//         } else {
//             alert("There is no such model");
//         }
//     }
// })

// let filterActive; //отфильтрованные результаты (?)

// filterList.addEventListener('click', (event) => { //отфильтрованный список годов. при нажатии на какой либо год выводятся только авто этого года 
...
// function load() {
//     const filterArr = []; //пустой массив для фильтра (в него попадут все года из объектов массива, даже повторяющиеся)

//     mustangs.map((item) => { //проходимся методом map по каждому объекту массивов
//         filterArr.push(item.year); //в массив запихиваем только значение объекта "год"
//         const block = ` 
//             <div class="item">
//                 <div class="img-holder">
//                     <img src="${item.image}" alt="mustang">
...
//                 </ul>
//             </div>
//         `; //создаём html-код для внесения каждого объекта массива в отдельный блок
//         contentHolder.insertAdjacentHTML('beforeend', block); //вставляем созданный блок в html-док
//     });

//     let list = [] //создаём финальный массив годов, которые отобразятся в фильтре (без повторяющихся)

//     const tempArray = filterArr.sort() //сортируем изначальный массив годов (сначала более старые годы)


//     for (let i = 0; i < tempArray.length; i++) { //проходимся по изначальному массиву годов (с повторяющимися годами)
//         if (tempArray[i + 1] !== tempArray[i]) { //условие: если каждый последующий год не равен предыдущему (т.е. уникальный, не дубликат) ->
//             list.push(tempArray[i]); // то запихиваем его в массив не-повторяющихся годов
//         }
//     }

//     list.map((list) => { //проходимся по массиву не-повторяющихся годов
//         const listItem = `<li>${list}</li>`; //для каждого года создаём лишку
//         filterList.insertAdjacentHTML('beforeend', listItem); //в каждую лишку вставляем свой год
//     });
// }

// function filter(item) { //функция фильтрации массива по годам (значению объекта)
//     const year = item.textContent; //получаем значение объекта массива (что буквально в нем записано) и записываем в константу
...
//     if(year != 'all') { //если функция понимает, что в li записано буквально 'all', она создаёт блок с инфой под весь список авто (т.е. без фильтра по годам, тк мы попросили вывести все годы)
//         mustangs.map((i) => {
//             if(i.year == year) {
//                 const block = `
//             <div class="item">
...
//             }
//         })
//     } else { //если функция видит под item какой-то определённый год из списка list (массив отфильтрованных годов, без дубликатов), то она выводит только карточки авто с этим годом
//         mustangs.map((item) => {
//             const block = `
//             <div class="item">
//                 <div class="img-holder">
